    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, attention_mask, past_key_values, labels, images,
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            if past_key_values is not None and vision_tower is not None and images is not None and input_ids.shape[
                1] == 1:
                # Safely get the sequence length from past_key_values
                past_seq_len = 0
                try:
                    if past_key_values and len(past_key_values) > 0:
                        last_layer_kv = past_key_values[-1]
                        if last_layer_kv is not None:
                            # past_key_values structure: tuple of (key, value) tuples per layer
                            # Try value tensor first (index 1), then key tensor (index 0)
                            if isinstance(last_layer_kv, (tuple, list)) and len(last_layer_kv) > 0:
                                tensor = last_layer_kv[1] if len(last_layer_kv) > 1 and last_layer_kv[1] is not None else last_layer_kv[0]
                                if tensor is not None and hasattr(tensor, 'shape') and len(tensor.shape) >= 2:
                                    past_seq_len = tensor.shape[-2]
                except (IndexError, AttributeError, TypeError):
                    # If we can't determine the length, use 0 (will result in attention_mask of length 1)
                    past_seq_len = 0
                
                attention_mask = torch.ones((attention_mask.shape[0], past_seq_len + 1),
