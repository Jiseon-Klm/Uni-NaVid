...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda"):
    ...
    # Check config to see if it's a vision model (llava/UniNaVid model)
    has_vision = 'vid' in model_name.lower()
    if not has_vision:
        try:
            config_path = model_path
            config = AutoConfig.from_pretrained(config_path)
            has_vision = (hasattr(config, 'model_type') and config.model_type == 'llava') or \
                        (hasattr(config, 'mm_vision_tower') and config.mm_vision_tower is not None)
        except:
            pass
    ...
    if hasattr(model, 'get_vision_tower'):
        try:
            ...
            has_vision_capabilities = vision_tower is not None
        except:
            pass
    
    if 'vid' in model_name.lower() or has_vision_capabilities:
